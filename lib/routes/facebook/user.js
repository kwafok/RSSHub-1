...
function getReactionsAndComments($) {
    ...
    $commentInput.remove();
    const $reactionAndComments = $('#add_comment_switcher_placeholder ~ *');
    $reactionAndComments.find('a').each((i, a) => {
        $(a).removeAttr('href'); // disable all link
        a.tagName = 'strong';
    });
    const reactionAndCommentsCollapsed = `<details><summary>Comments</summary>${$reactionAndComments.toString()}</details>`;
    return reactionAndCommentsCollapsed;
}
